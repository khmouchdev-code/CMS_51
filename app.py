...
class Member(db.Model):
    ...
    records = db.relationship('CollectionRecord', backref='member', lazy=True, cascade='all, delete-orphan')
...
def save_profile_picture(file, member_id):
    if file and allowed_file(file.filename):
        ...
        filename = f"member_{member_id}_{secrets.token_hex(8)}.{ext}"
...
def dashboard():
    ...
    base_query = CollectionRecord.query.filter_by(member_id=session['user_id']).filter(EXCLUDE_FEE_ONLY)

    today_records = base_query.filter(CollectionRecord.date == today).order_by(CollectionRecord.call_time.asc()).all()
    today_sampling = sum(r.total_sampling for r in today_records)
    today_entries_count = len(today_records)
    today_patients = db.session.query(db.func.sum(CollectionRecord.patient_count)).filter(
        CollectionRecord.member_id == session['user_id'],
        CollectionRecord.date == today,
        EXCLUDE_FEE_ONLY
    ).scalar() or 0
    today_moto_fee = db.session.query(db.func.sum(CollectionRecord.moto_fee)).filter(
        CollectionRecord.member_id == session['user_id'],
        CollectionRecord.date == today,
        EXCLUDE_FEE_ONLY
    ).scalar() or 0
    today_far_away_fee = db.session.query(db.func.sum(CollectionRecord.far_away_fee)).filter(
        CollectionRecord.member_id == session['user_id'],
        CollectionRecord.date == today,
        EXCLUDE_FEE_ONLY
    ).scalar() or 0
    today_total_fees = today_moto_fee + today_far_away_fee

    last_7_days = []
    for i in range(6, -1, -1):
        day = today - timedelta(days=i)
        day_records = base_query.filter(CollectionRecord.date == day).all()
        day_total = sum(r.total_sampling for r in day_records)
        last_7_days.append({
    ...
    total_all_sampling = db.session.query(db.func.sum(CollectionRecord.total_sampling)).filter(
        CollectionRecord.member_id == session['user_id'],
        EXCLUDE_FEE_ONLY
    ).scalar() or 0
    total_patients = db.session.query(db.func.sum(CollectionRecord.patient_count)).filter(
        CollectionRecord.member_id == session['user_id'],
        EXCLUDE_FEE_ONLY
    ).scalar() or 0
    ...
    filtered_records = query.order_by(CollectionRecord.date.desc(), CollectionRecord.call_time.asc()).all()
    filtered_total_records = len(filtered_records)
    filtered_total_sampling = sum(r.total_sampling for r in filtered_records)
    filtered_total_patients = sum(r.patient_count for r in filtered_records)
    filtered_total_fees = sum((r.moto_fee or 0) + (r.far_away_fee or 0) for r in filtered_records)
    ...
    return render_template('dashboard.html',
                           ...
                           today_sampling=today_sampling,
                           today_patients=today_patients,
                           today_entries_count=today_entries_count,
                           today_moto_fee=today_moto_fee,
                           today_far_away_fee=today_far_away_fee,
                           today_total_fees=today_total_fees,
                           last_7_days=last_7_days,
                           total_records=total_records,
                           total_all_sampling=total_all_sampling,
...
def report():
    ...
    records = query.order_by(CollectionRecord.date.desc(), CollectionRecord.call_time.asc()).all()
    ...
    facilities = set(r.location for r in records if r.location)
    doctors = set(r.doctor for r in records if r.doctor)
    ...
    for r in records:
        ...
        if loc not in location_summary:
            location_summary[loc] = {'patients': 0, 'specimens': 0, 'count': 0}
    ...
    for r in records:
        ...
        if d not in daily_summary:
            daily_summary[d] = {'patients': 0, 'specimens': 0, 'count': 0}
    ...
    if session.get('is_admin'):
        ...
        for r in records:
            mid = r.member_id
            if mid not in collector_data:
                member_obj = db.session.get(Member, mid)
                collector_data[mid] = {
                    'full_name': member_obj.full_name if member_obj else 'Unknown',
                    'patients': 0,
                    'specimens': 0,
                    'days': set()
                }
    ...
    all_facilities = sorted(set(r.location for r in records if r.location))
...
def export_csv():
    ...
    records = query.order_by(CollectionRecord.date.desc()).all()
...
def profile():
            ...
            if 'profile_picture' in request.files:
                ...
                if file and file.filename:
                    if member.profile_picture != 'default.png':
                        old = os.path.join(app.config['UPLOAD_FOLDER'], member.profile_picture)
    ...
    total_sampling = db.session.query(db.func.sum(CollectionRecord.total_sampling)).filter(
        CollectionRecord.member_id == member.id,
        EXCLUDE_FEE_ONLY
    ).scalar() or 0
    member_since = member.created_at.strftime('%B %d, %Y') if member.created_at else 'N/A'
    return render_template('profile.html', member=member, total_records=total_records, total_sampling=total_sampling,
...
def members_list():
    ...
    for m in members:
        ...
        last_record = base_q.order_by(
            CollectionRecord.date.desc(), CollectionRecord.created_at.desc()
        ).first()
...
def member_details(member_id):
    ...
    records = base_q.order_by(CollectionRecord.date.desc()).all()
...
def admin_panel():
    ...
    total_samples_all = db.session.query(db.func.sum(CollectionRecord.patient_count)).filter(
        EXCLUDE_FEE_ONLY).scalar() or 0
    recent_logs = SystemLog.query.order_by(SystemLog.timestamp.desc()).limit(50).all()

    member_stats = {}
    for m in members:
        m_total_sampling = db.session.query(db.func.sum(CollectionRecord.total_sampling)).filter(
            CollectionRecord.member_id == m.id,
            EXCLUDE_FEE_ONLY
        ).scalar() or 0
        m_total_samples = db.session.query(db.func.sum(CollectionRecord.patient_count)).filter(
            CollectionRecord.member_id == m.id,
            EXCLUDE_FEE_ONLY
        ).scalar() or 0
        member_stats[m.id] = {
...
def admin_delete_member(member_id):
    ...
    # Broadcast offline status before deletion
    broadcast_status_update(member.id, False, {
        'full_name': member.full_name,
        'username': member.username
    })
...
def admin_report():
    ...
    for m in members:
        ...
        total_moto = db.session.query(db.func.sum(CollectionRecord.moto_fee)).filter(
            CollectionRecord.member_id == m.id,
            EXCLUDE_FEE_ONLY
        ).scalar() or 0
        total_far = db.session.query(db.func.sum(CollectionRecord.far_away_fee)).filter(
            CollectionRecord.member_id == m.id,
            EXCLUDE_FEE_ONLY
        ).scalar() or 0
...
def init_db():
    with app.app_context():
        # Check if last_activity column exists, if not add it with correct PostgreSQL syntax
        try:
            ...
            if 'member' in inspector.get_table_names():
                ...
                if 'last_activity' not in columns:
                    ...
                    if 'postgresql' in db_uri or 'psycopg2' in db_uri:
                        db.session.execute(text("ALTER TABLE member ADD COLUMN last_activity TIMESTAMP"))
                    else:
                        db.session.execute(text("ALTER TABLE member ADD COLUMN last_activity DATETIME"))
                    db.session.commit()
                    print("✅ Added last_activity column")
                else:
                    print("✅ last_activity column already exists")
        except Exception as e:
            print(f"⚠️ Could not add last_activity column: {e}")
            db.session.rollback()
